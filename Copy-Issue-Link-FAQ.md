...
### Can I copy issue data from the Jira command palette?

Yes, Copy Issue Link is available in [Jira's command palette](https://support.atlassian.com/jira-software-cloud/docs/what-is-the-command-palette/). You can copy the issue title + link from the command palette, or use your custom templated buttons to copy, for example, the issue key all with just your keyboard.

Open the command palette with **command + K** on Mac or **Ctrl + K** on Windows. Then search for `copy issue link` (or `cc` for short) to bring up the Copy Issue Link app. On Firefox and Chrome, you can even use the number keys on your keyboard to quickly copy issue properties to your clipboard without using your mouse:

![copy-issue-link-command-palette](/images/copy-issue-link-command-palette.gif)
